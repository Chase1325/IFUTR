...
# This code has been modified and is originally from
# http://svn.modlabupenn.org/mod-ros-pkg/vicon/python-libs/viconreader.py
...
class ViconReader(object):
    ...
    >>> for pkt,_ in zip(V.stream(),range(100)):
...
Usage: %s {options} filename
  ...
  """ % sys.argv[0])
    ...
    print
    "Recording from %s:%d to %s.dcr, %s.raw" % (host, port, fn, fn)
    V = ViconReader()
